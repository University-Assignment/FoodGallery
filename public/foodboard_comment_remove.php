...
if ($id >= 0) {
  $check = $mysqli->query("SELECT * FROM foodboard_comment WHERE aid = ".$_SESSION['id']." AND id = ".$id."");
  if ($check->num_rows > 0) {
    $sql = $mysqli->query("DELETE FROM foodboard_comment WHERE aid = ".$_SESSION['id']." AND id = ".$id."");
    if ($sql) {
      ?>
